        ...
        toolbar: [{
            ...
            handler: function () {
                ...
                if (rows.length > 0) {
                    ...
                    if (fgid == "2") {
                        msgShow("提示", "您没有权限进行操作,已结提交审核不能进行删除！");
                        return;
                    }
                    $.messager.confirm('询问提示', '您确定要删除选中的编号为 [' + rows[0].id + '] 的信息吗？', function (data) {
                        if (data) {
                            $.get("../Ashx/Xmbeian.ashx?action=del", { id: rows[0].id }, function (data) {
                                slide("提示", data);
                                $('#tdg').datagrid('reload');
            ...
            handler: function () {
                ...
                if (rows.length > 0) {
                    ...
                    var fgid = rows[0].beiyong;
                    if (fgid == "2") {
                        msgShow("提示", "您没有权限进行操作,已结提交审核不能进行修改！");
                        return;
                    }
                    $('#newAdd').window({ title: "修改项目备案信息" });
                    $('#newAdd').window('open');
                   // OnEmptyTextClick();
                    $.get("../Ashx/Xmbeian.ashx?action=by", { Id: rows[0].id }, function (data) {
                        $("#txtOrgCode").combotree("setValue", data.orgcode);


                        $("#txtBidName").textbox("setValue", data.cqname);
                        $("#txtProperties").combotree("setValue", data.cqleixing);
                        $("#txtOwnership").textbox("setValue", data.cqquanshu);
                        $("#txtguimo").textbox("setValue", data.guimo);
                        $("#comgmdw").textbox("setValue", data.danwei);
                        $("#txtcrf").textbox("setValue", data.churangfang);
                        $("#txtcrfxz").textbox("setValue", data.churangfangxingzhi);
                        $("#txtsrf").textbox("setValue", data.shourangfang);
                        $("#txtsrfxz").textbox("setValue", data.shourangfangxingzhi);
                        $("#txtStandardMode").combotree("setValue", data.jiaoyifangshi);
                        $("#txtStartDate").datebox("setValue", data.hetongksrq);
                        $("#txtEndDate").datebox("setValue", data.hetongjsrq);
                        $("#txtdanjia").textbox("setValue", data.chengjiaodanjia);
                        $("#txtzongjia").textbox("setValue", data.changjiaozongjia);
                        $("#txtzffs").textbox("setValue", data.zhifufangshi);
                        $("#txtbeizhu").textbox("setValue", data.beizhu);
                        $("#txtId").val(data.id);
...
jQuery(function ($) {
    $("#btnCheckAdd").click(function () {
        if ("" != $("#txtSay").val()) {
            $.get("../Ashx/Xmbeian.ashx?action=shenhe", { AcceptSay: $("#txtSay").val(), BidId: $('#tdg').datagrid('getSelections')[0].id, UpManager: $('#tdg').datagrid('getSelections')[0].UpManager, OrgCode: $('#tdg').datagrid('getSelections')[0].oCode, DepaStatusId: $('#tdg').datagrid('getSelections')[0].DepaStatusId }, function (data) {
