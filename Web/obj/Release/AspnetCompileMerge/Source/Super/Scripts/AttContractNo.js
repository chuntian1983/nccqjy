jQuery(function ($) {
    ...
    $('#WinBidContractScan').window('close');
});
...
{ field: 'Admissibility', title: '受理申请号', width: 80, align: 'left', sortable: true, formatter: function (value, row, index) { return "HJNS" + row.Admissibility; } },
...
{ field: 'ContractNo', title: '合同号码', width: 80, align: 'left', sortable: true },
...
{ field: 'DepaStatus', title: '分管部门', width: 80, align: 'left', sortable: true }
        ...
        toolbar: [{
            ...
            handler: function () {
                ...
                if (rows.length > 0) {
                    $.get("../Ashx/AttAuRep.ashx?action=ConNo", { Id: rows[0].Id }, function (data) {
            ...
            handler: function () {
                var rows = $('#tdg').datagrid('getSelections');
                if (rows.length > 0) {
                    $('#WinBidContractScan').window('open');
                    $("#BidContractScanPage").attr("src", "BidContrScan.aspx?FK_BidId=" + rows[0].Id);
                } else {
                    msgShow("提示", "您还没有选中一列信息？", "question");
                }
            }
