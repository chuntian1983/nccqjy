...
function GridView() {
    $('#tdg').datagrid({
        ...
        nowrap: true,
        striped: true,
        url: '../ashx/MemberType.ashx?action=paging',
        remoteSort: false,
        sortName: 'Id',
        singleSelect: true,
        sortOrder: 'asc',
...
{ field: 'TypeCertificationName', title: '会员类型名称', width: 150, align: 'left', sortable: true },
        ...
        toolbar: [{
            ...
            handler: function () {
                ...
                if (rows.length > 0) {
                    ...
                    $('#newAdd').window('open');
                    EmptyTextClick();
                    $.get("../ashx/MemberType.ashx?action=by", { Id: rows[0].Id }, function (data) {
                        $("#txtId").val(data.Id);
                        $("#txtTypeCertificationName").val(data.TypeCertificationName); 
                    }, "json");
                    $("#btnAdd").hide();
                    $("#btnEdit").show();
                } else {
...
function OnUpdateClick() {
    $('#forms').form('submit', {
        url: '../ashx/MemberType.ashx?action=up',
        async: false,
        onSubmit: function () {
...
function OnCreateClick() {
    $('#forms').form('submit', {
        url: '../ashx/MemberType.ashx?action=add',
        async: false,
        onSubmit: function () {
            return $(this).form('validate');
        },
