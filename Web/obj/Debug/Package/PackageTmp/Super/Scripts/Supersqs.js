...
function GridView() {
    $('#tdg').datagrid({
        ...
        nowrap: true,
        striped: true,
        url: '../Ashx/BidH.ashx?' + $.param({ action: "dyzt", DepaStatus: 7, dyzt: 3 }),
        remoteSort: false,
        sortName: 'id',
        singleSelect: true,
        sortOrder: 'desc',
...
{ field: 'dyzt', title: '打印申请', width: 80, align: 'left', sortable: true, formatter: function (value, row, index) { return row.dyzt == 2 ? "<span style='color:blue;'>分管领导审核</span>" : "<span style='color:red;'>主管领导审核<span>"; } },
{ field: 'binid', title: '编号', width: 80, align: 'left', sortable: true, hidden: true },
{ field: 'Name', title: '出让方姓名', width: 80, align: 'left', sortable: true },
        ...
        toolbar: [{
            ...
            handler: function () {
                ...
                if (rows.length > 0) {
                    $("#txtid").val(rows[0].id);
                    $('#newAdd').window('open');
                    $.get("../Ashx/BidH.ashx?action=sqs", { Id: rows[0].binid }, function (data) {
                        $("#txtbeizhu").textbox('setValue', data);
...
jQuery(function ($) {
    $("#btnshenhe").click(function () {
        var tid = $("#txtid").val();
        $.get("../Ashx/BidH.ashx", { action: "changezg", id: tid },
          function (data) {
              if (data == "0") {
