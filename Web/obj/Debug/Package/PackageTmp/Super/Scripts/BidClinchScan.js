...
function GridView() {
    $('#tdg').datagrid({
        nowrap: true,
        striped: true,
        url: '../Ashx/BidClinchSca.ashx?' + $.param({ action: "paging", FK_BidId: Request.QueryString("FK_BidId") }),
        remoteSort: false,
        sortName: 'Id',
        singleSelect: true,
        sortOrder: 'desc',
        ...
        onDblClickRow: function (rowIndex, rowData) {
           $(function () {
                parent.OnShowScan(rowData.ScanName, "../Scan/biscan/" + rowData.FK_BidId + "/" + rowData.ScanUrl);
            });
        },
        columns: [[
{ field: 'Id', title: '序号', width: 40, align: 'left', sortable: true },
{ field: 'ScanName', title: '附件文件名称', width: 150, align: 'left', sortable: true },
{ field: 'FK_BidId', title: '标代码', width: 80, align: 'left', sortable: true, hidden: true },
{ field: 'ScanUrl', title: '附件文件路径名称', width: 150, align: 'left', sortable: true, hidden: true },
{ field: 'Editor', title: '上传人', width: 100, align: 'left', sortable: true },
{ field: 'CreateDate', title: '上传时间', width: 130, align: 'left', sortable: true }
]],
        toolbar: [
        {
            id: 'btnadd',
            ...
            handler: function () {
                ...
                if (rows.length > 0) {
                    $.messager.confirm('询问提示', '您确定要删除选中的编号为 [' + rows[0].Id + '] 的信息吗？', function (data) {
                        if (data) {
                            $.get("../Ashx/BidClinchSca.ashx?action=del", { id: rows[0].Id }, function (data) {
                                slide("提示", data);
                                $('#tdg').datagrid('reload');
            ...
            handler: function () {
                ...
                if (rows.length > 0) {
                    ...
                    $('#newAdd').window('open');
                    OnEmptyTextClick();
                    $.get("../Ashx/BidClinchSca.ashx?action=by", { Id: rows[0].Id }, function (data) {
                        $("#txtId").val(data.Id);
...
jQuery(function ($) {
    $("#btnEdit").click(function () {
        $('#forms').form('submit', {
            url: '../Ashx/BidClinchSca.ashx?action=up',
            async: false,
            onSubmit: function () {
...
jQuery(function ($) {
    $("#btnAdd").click(function () {
        $('#forms').form('submit', {
            url: '../Ashx/BidClinchSca.ashx?action=add',
            async: false,
            onSubmit: function () {
                return $(this).form('validate');
            },
