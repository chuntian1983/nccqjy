...
namespace NCPEP.Dal
{
    ...
    public partial class T_BidTrans
    {
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(NCPEP.Model.T_BidTrans model)
        {
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(NCPEP.Model.T_BidTrans model)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        /// 得到一个对象实体
        /// </summary>
        public NCPEP.Model.T_BidTrans GetModel(int Id)
        {

            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 Id,FK_BidId,FK_LiceTranId,TradingStatus,Editor,CreateDate,ApplyDate from T_BidTrans ");
            strSql.Append(" where Id=@Id");
            SqlParameter[] parameters = {
					new SqlParameter("@Id", SqlDbType.Int,4)
			};
            ...
            NCPEP.Model.T_BidTrans model = new NCPEP.Model.T_BidTrans();
        ...
        /// 得到一个对象实体
        /// </summary>
        public NCPEP.Model.T_BidTrans DataRowToModel(DataRow row)
        {
            NCPEP.Model.T_BidTrans model = new NCPEP.Model.T_BidTrans();
            if (row != null)
            {
                ...
                if (row["TradingStatus"] != null && row["TradingStatus"].ToString() != "")
                {
                    model.TradingStatus = int.Parse(row["TradingStatus"].ToString());
                }
                if (row["Editor"] != null)
                {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select Id,FK_BidId,FK_LiceTranId,TradingStatus,Editor,CreateDate,ApplyDate ");
            strSql.Append(" FROM T_BidTrans ");
            if (strWhere.Trim() != "")
            {
        ...
        public DataSet GetList(int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(" Id,FK_BidId,FK_LiceTranId,TradingStatus,Editor,CreateDate,ApplyDate ");
            strSql.Append(" FROM T_BidTrans ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) FROM T_BidTrans ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
        {
            ...
            strSql.Append(")AS Row, T.*  from T_BidTrans T ");
