...
namespace NCPEP.Dal
{
    public class StandardTypeDal
    {
        private dynamic db = null;
        public StandardTypeDal()
        {
            db = new MsSqlHelper();
        }
        //
        ...
        public dynamic GetAllList(string id)
        {
            try
            {
                string strSql = " select Id,StandardTypeName,Editor,CreateDate from T_StandardType ";
        ...
        public dynamic GetAllList(string sort, string order)
        {
            try
            {
                string strSql = string.Format(" select Id,StandardTypeName,Editor,CreateDate from T_StandardType order by {0} {1}", sort, order);
                return db.ExecuteDataTable(strSql);
            }
            catch { throw; }
        }
        ...
        public dynamic GetModel(int Id)
        {
            try
            {
                dynamic model = null;
                StringBuilder strSql = new StringBuilder();
                strSql.Append(" select Id,StandardTypeName,Editor,CreateDate from T_StandardType ");
                strSql.Append(" where Id=@Id");
                ...
                {
                    model = new StandardType();
                    if (read.Read())
                    {
                        try { model.Id = int.Parse(read["Id"].ToString()); }
                        catch { }
                        model.StandardTypeName = read["StandardTypeName"].ToString();
        ...
        {
            SqlParameter[] parameters = {
					new SqlParameter("@StandardTypeName", SqlDbType.NVarChar,50),
					new SqlParameter("@Editor", SqlDbType.NVarChar,50),
					new SqlParameter("@CreateDate", SqlDbType.DateTime),
					new SqlParameter("@Id", SqlDbType.Int,4)};
            parameters[0].Value = model.StandardTypeName;
