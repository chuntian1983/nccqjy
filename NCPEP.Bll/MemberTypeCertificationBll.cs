...
namespace NCPEP.Bll
{
    public class MemberTypeCertificationBll
    {
        private MemberTypeCertificationDal dal = null;
        private dynamic adminUser = null;
        public MemberTypeCertificationBll()
        {
            dal = new MemberTypeCertificationDal();
        }
        public string ProcessRequest(HttpContext context)
        {
            ...
            {
                case "paging":
                    returnDate = JsonHelper<MemberTypeCertification>.JsonDataTable(GetAllList(context), "rows");
                    break;
                case "add":
                ...
                case "combo":
                    ...
                    break;
                case "by":
                    returnDate = JsonHelper<MemberTypeCertification>.JsonWriter(GetById(context));
                    break;
                default:
                    returnDate = "请求错误!";
                    break;
        ...
        private dynamic GetById(HttpContext context)
        {
            try
            {
                SysLogBll.Create("会员类型", "返回会员类型信息操作", adminUser.AdminName);
                int id = 0;
        ...
        private string GetComboData()
        {
            try
            {
                SysLogBll.Create("会员类型", "返回会员类型combo数据操作", adminUser.AdminName);
        ...
        private string Update(HttpContext context)
        {
            try
            {
                SysLogBll.Create("会员类型", "更新会员类型操作", adminUser.AdminName);
        ...
        private string Create(HttpContext context)
        {
            try
            {
                SysLogBll.Create("会员类型", "添加会员类型操作", adminUser.AdminName);
        ...
        private DataTable GetAllList(HttpContext context)
        {
            try
            {
                ...
                SysLogBll.Create("会员类型", "获取所有的会员类型操作", adminUser.AdminName);
