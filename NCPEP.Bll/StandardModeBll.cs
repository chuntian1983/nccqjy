...
namespace NCPEP.Bll
{
    public class StandardModeBll
    {
        private dynamic dal = null;
        private dynamic adminUser = null;
        public StandardModeBll()
        {
            dal = new StandardModeDal();
        }
        public string ProcessRequest(HttpContext context)
        {
            ...
            {
                case "paging":
                    returnDate = JsonHelper<StandardMode>.JsonDataTable(GetAllList(context), "rows");
                    break;
                case "add":
                ...
                case "combo":
                    ...
                    break;
                case "by":
                    returnDate = JsonHelper<StandardMode>.JsonWriter(GetById(context));
                    break;
                default:
                    returnDate = "请求错误!";
                    break;
        ...
        private dynamic GetById(HttpContext context)
        {
            try
            {
                SysLogBll.Create("挂牌方式", "返回单个挂牌方式信息操作", adminUser.AdminName);
                int id = 0;
        ...
        private string GetComboData()
        {
            try
            {
                SysLogBll.Create("挂牌方式", "返回挂牌方式combo数据操作", adminUser.AdminName);
        ...
        private string Update(HttpContext context)
        {
            try
            {
                SysLogBll.Create("挂牌方式", "更新挂牌方式操作", adminUser.AdminName);
        ...
        private string Create(HttpContext context)
        {
            try
            {
                SysLogBll.Create("挂牌方式", "添加挂牌方式操作", adminUser.AdminName);
        ...
        private DataTable GetAllList(HttpContext context)
        {
            try
            {
                ...
                SysLogBll.Create("挂牌方式", "获取所有的挂牌方式操作", adminUser.AdminName);
        ...
        {
            dynamic model = new StandardMode();
            model.StandardModeName = context.Request.Form["StandardModeName"];
